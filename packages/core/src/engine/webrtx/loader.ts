...
export interface ShaderSourceAndRecordData {
...
export type MaterialDescription = HitGroupMaterialShadersDescription | HitGroupMaterialShadersDescription[];
...
interface AABBGeometryDescription {
  type: 'aabb';
  intersection: string; // TODO: does it make sense to support different shaders per ray type?
  aabb: AABB;
}
...
export interface SceneDescription {
  version: string;
  rayTypes: number;
  rayGen: ShaderSourceAndRecordData;
  // may define per ray type
  rayMiss?: string | ShaderSourceAndRecordData[];
  bindings?: Binding[];
  materials: {
...
async function loadGeometryButWithoutGPUBuffer(desc: GeometryDescription): Promise<GPURayTracingAccelerationGeometryDescriptor> {
  switch (desc.type) {
    case 'triangles': {
      ...
      return {
        ...
        type: 'triangles',
        vertex: {
          buffer: buffer as unknown as GPUBuffer, // cast ok: will allocate and replace GPUBuffer later,
          format: 'float32x3',
          stride: 3 * 4,
        },
        index: index && {
...
export default class SceneLoader {
  async load(device: GPUDevice, sceneDescription: SceneDescription): Promise<LoadedSceneResult> {
    ...
    async function addShader(shaderStage: _GPUShaderStageRTX, shaderFileInfo: string): Promise<_ShaderStageDesc> {
      const stage = shaderFilenameToStage.get(shaderFileInfo);
      if (stage) {
        return stage;
      }
      const desc = {
        // manually include common.glsl content
        glslCode: shaderFileInfo.replace('#include "common.glsl"', `${commonGlsl}\n`),
        stage: shaderStage,
        entryPoint: 'main', // TODO: configurable
        _index: shaderFilenameToStage.size,
      };
      shaderFilenameToStage.set(shaderFileInfo, desc);
      return desc;
    }
    ...
    await visitSceneInstanceAsync(sceneDescription, async (instanceDesc, blasDesc) => {
      ...
      if (!blas) {
        const geometries = await Promise.all(blasDesc.geometries.map(async gDesc => await loadGeometryButWithoutGPUBuffer(gDesc)));
        blas = {
