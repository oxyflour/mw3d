...
export default function wrap<T extends ApiDefinition>({ num, api, fork, send, recv }: {
    ...
    if (cluster.isWorker) {
        process.addListener('message', async data => {
            const { id, entry, args } = data as { id: string, entry: string[], args: any[] },
